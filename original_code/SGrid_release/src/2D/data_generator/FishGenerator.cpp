...
void FishGenerator::update() {
  ...
  for (std::vector<Unit*>::iterator uit = unitSeq.begin(); uit != unitSeq.end();
      uit++) {
    Unit* unit = *uit;
    ...
    if (unitPosIndex->isMovingJoin() && (newVx != oldVx || newVy != oldVy)) {
      ...
      mvJoin =
          (MovingSpatialJoin2D<Unit, UnitKey, UnitAgg2D, UnitRG2D>*) unitPosIndex;

      mvJoin->beforeUpdateVelocity(unit, unit->getX(), unit->getY(),
          newVx * unit->getSpeed(), newVy * unit->getSpeed());
    }
...
void FishGenerator::genFish() {
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    unitSeq[i] = new Unit(id, x, y, vx, vy, speed, informed,
        goals[goalNum]->getGx(), goals[goalNum]->getGy());
...
//    }
    ...
    // Applies the effects calculated in the previous step (query).
    // Since it does not update the index, it should not be
