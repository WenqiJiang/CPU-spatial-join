...
JoinMethod* getJoinByName(const char* name) {
  ...
  if (strcmp(name, NaiveSpatialIndex2D<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new NaiveSpatialIndex2D<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, NaiveMovingIndex2D<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new NaiveMovingIndex2D<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, BinarySearch2D<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new BinarySearch2D<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, RTree<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new RTree<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, CRTree<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new CRTree<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, Stripes<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    //TODO: replace maxInt by the parameter passed from datagen
    IndexType* index = new Stripes<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, TPRTree<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new TPRTree<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, SimpleGrid<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new SimpleGrid<Point2D, Key2D, SumAgg2D>(-1, 30000, 4 );
    return getJoinMethod(index);
  } else if (strcmp(name, LinearizedKDTrie<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new LinearizedKDTrie<Point2D, Key2D, SumAgg2D>();
    return getJoinMethod(index);
  } else if (strcmp(name, SGrid<Point2D, Key2D, SumAgg2D>::getClassName()) == 0) {
    IndexType* index = new SGrid<Point2D, Key2D, SumAgg2D>(SGRID_DFLT_CPS, SGRID_DFLT_BS);
    return getJoinMethod(index);
  }
  ...
  // you are addint a new join method, return a new instance directly.
  ...
  else if (strcmp(name, StaticNestedLoops2D<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    throwUsage(
        "Cannot Initialize NestedLoops join directly.\n \
                        Use the name of the static index instead.");
  } else if (strcmp(name, MovingNestedLoops2D<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    throwUsage(
        "Cannot Initialize NestedLoops join directly.\n \
                        Use the name of the static index instead.");
  } else if (strcmp(name, GenericPlaneSweep2D<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    ...
    return getJoinMethod(new GenericPlaneSweep2D<Point2D, Key2D, SumAgg2D, RegionGen2D>(ss));
  } else if (strcmp(name, ForwardPlaneSweep<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    return getJoinMethod(new ForwardPlaneSweep<Point2D, Key2D, SumAgg2D, RegionGen2D>());
  } else if (strcmp(name, ForwardPBSM2D<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    return getJoinMethod(new ForwardPBSM2D<Point2D, Key2D, SumAgg2D, RegionGen2D>());
  }

  else if (strcmp(name, GenericPBSM2D<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    return getJoinMethod(new GenericPBSM2D<Point2D, Key2D, SumAgg2D, RegionGen2D>());
  } else if (strcmp(name, SynchTraversal<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
  ...
  } else if (strcmp(name, TrivialContinuousJoin<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName())
      == 0) {
    ContJoinType *j;
    j = new TrivialContinuousJoin<Point2D, Key2D, SumAgg2D, RegionGen2D>();
    return getJoinMethod(j);
  } else if (strcmp(name, AEJoin<Point2D, Key2D, SumAgg2D, RegionGen2D>::getClassName()) == 0) {
    ContJoinType *j;
...
Workload2D* getWorkload(const char* traceFile) {
  ...
  LOG(DEBUG) << "Parsing file " << traceFile << " as a " << (binary ? "binary " : "text ") << "file"
      << std::endl;
...
void compareTrace(JoinMethod* i1, JoinMethod* i2, const char* tracefile) {
  ...
  try {
    ...
    LOG(INFO) << "Testing " << i1->getName() << " and " << i2->getName() << " on trace " << tracefile
        << std::endl;
...
//  IndexLog::setLevel(DEBUG);

  Log::open("-"); //for asserts;
//  Log::open("log.txt"); //for asserts;
//  Log::setLevel(Log::DEBUG);
  ...
  for (int argNum = 2; argNum < argc; argNum++) {
    ...
    if (((v = suffixAfterMatch(arg, "--index=")) != 0) || ((v = suffixAfterMatch(arg, "--index1=")) != 0)) {
