...
public:
  void init() {
    ...
    addIndex(getJoinMethod(new SimpleGrid<Point2D, Key2D, SumAgg2D>(GRID_DFLT_CPS, 30000, GRID_DFLT_BS)));

    addIndex(getJoinMethod(new SGrid<Point2D, Key2D, SumAgg2D>(SGRID_DFLT_CPS, SGRID_DFLT_BS)));
