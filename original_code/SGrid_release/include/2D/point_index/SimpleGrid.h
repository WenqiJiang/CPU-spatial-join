...
#define GRID_DFLT_CPS 13 // default number of cells per side
#define GRID_DFLT_BS 4 // default bucket size
...
public:
  ...
  SimpleGrid(int cellsPerSide = GRID_DFLT_CPS, int spaceSide = 22361, uint32_t bucketSize = GRID_DFLT_BS,
      int numAgents = 50000);
...
template<typename U, typename K, typename A>
uint32_t SimpleGrid<U, K, A>::Bucket::bucketSize = GRID_DFLT_BS;
...
template<typename U, typename K, typename A>
void SimpleGrid<U, K, A>::Bucket::enumerateOverRegion(IndexCallBack<U>* cb, RType* r) {
  Entry* node = _head;

  while (node != NULL) {
    if (r->template containsPoint<U, K>(node->data)) {
      LOG(DEBUG) << "[SimpleGrid enumOverRegion] reporting: " << *(node->data) << std::endl;
...
template<typename U, typename K, typename A>
void SimpleGrid<U, K, A>::GridCell::enumerateOverRegion(IndexCallBack<U>* cb, RType* r) {
  if (bucketHead != NULL) {
...
template<typename U, typename K, typename A>
SimpleGrid<U, K, A>::SimpleGrid(int cellsPerSide, int spaceSide, uint32_t bucketSize, int numAgents) {
  ...
  cells = (GridCell*) AlignedMem::allocateClear((sizeof(GridCell) * this->cellsPerSide * this->cellsPerSide),
      64);
...
//  printf("Simple Grid configuration:\n");
//  printf(" cpc = %d\n", this->cellsPerSide );
//  printf(" bs = %d\n", bucketSize );
//  printf(" cellSize = %f\n", cellSize );
//  printf(" maxSpaceSide = %d\n", spaceSide );
...
template<typename U, typename K, typename A>
void SimpleGrid<U, K, A>::enumerateOverRegion(IndexCallBack<U>* cb, RType* r) {
  for (int i = 0; i < cellsPerSide; ++i) {
    for (int j = 0; j < cellsPerSide; ++j) {
