  ...
  public:
    ...
    friend std::ostream& operator<<(std::ostream &out, const Node& node) {
      out << "RTree::Node(" << std::setiosflags(std::ios::fixed)
          << std::setprecision(2) << (node.isLeaf() ? "leaf" : "internal")
          << ", " << node.getCount();
    }
  };
...
template<typename U, typename K, typename A>
void RTree<U, K, A>::Node::enumerateOverRegion(IndexCallBack<U>* cb, RType* r) {
  nodeVisits++;
...
void RTree<U, K, A>::bulkLoad(std::vector<U*>* agents) {
  ...
  for (; i < (int) agents->size() - (int) numInLeaf; i += numInLeaf) {
    ...
    for (int k = i; k < i + numInLeaf; k++) {
      RType r(K::getKey0((*agents)[k]), K::getKey0((*agents)[k]),
          K::getKey1((*agents)[k]), K::getKey1((*agents)[k]));
...
void RTree<U, K, A>::bulkLoadQueries(std::vector<U*>* queryPoints, RG* rg) {
  ...
  QuerySort<U, RType, typename K::KeyType0, RType::low0val>::quicksort(queries,
      queryPoints);
  ...
  for (; i < (int) queryPoints->size() - Sn; i += Sn) {
    QuerySort<U, RType, typename K::KeyType1, RType::low1val>::quicksort(
        queries, queryPoints, i, i + Sn - 1);
  }

  QuerySort<U, RType, typename K::KeyType1, RType::low1val>::quicksort(queries,
      queryPoints, i, numQueries - 1);
