...
private:
  ...
  inline std::vector<Unit*>& getUnits() {
    return unitSeq;
  }
  inline std::vector<Goal*>& getGoals() {
    return goals;
  }
...
public:
  ...
  inline void setNumGoals(int numGoals) {
  ...
  static FishGenerator* createFishGenerator() {
    SpatialJoin2D<Unit, UnitKey, UnitAgg2D, UnitRG2D>* unitJoin = new JoinType<
        Unit, UnitKey, UnitAgg2D, UnitRG2D>();
  ...
  static FishGenerator* createFishGenerator() {
    IndexType<Unit, UnitKey, UnitAgg2D>* unitIndex = new IndexType<Unit,
        UnitKey, UnitAgg2D>();
    ...
    if (unitIndex->isMovingIndex()) {
      ...
      unitJoin = new MovingNestedLoops2D<Unit, UnitKey, UnitAgg2D, UnitRG2D>(
          movingIndex);
    } else {
      StaticSpatialIndex2D<Unit, UnitKey, UnitAgg2D>* staticIndex =
          (StaticSpatialIndex2D<Unit, UnitKey, UnitAgg2D>*) unitIndex;

      unitJoin = new StaticNestedLoops2D<Unit, UnitKey, UnitAgg2D, UnitRG2D>(
          staticIndex);
    }
