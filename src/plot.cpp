...
//        DBUTL_ASSERT(strcmp(tr1->getParamName(), tr2->getParamName()) == 0);
  return (tr1->getIndexNum() < tr2->getIndexNum()
      || ((tr1->getIndexNum() == tr2->getIndexNum()) && tr1->getWorkloadNum() < tr2->getWorkloadNum()));
}
...
MapType* createMap(const char* dirName, char* experName
...
//        int totSize = first->iParams->size() + first->wParams->size();
  ...
  for (it = trMap->begin(); it != trMap->end(); it++) {
    ...
    for (vIt = vec->begin(); vIt != vec->end(); vIt++) {
      ...
      std::cout << "name: " << tr->getIndexName() << " numResults: " << tr->getAvgTrialNumResults()
          << std::endl;
      ...
      if (targetTrial == -1) {
        fprintf(out, "buildMin: %lf queryMin: %lf updateMin: %lf ", baMin, qaMin, uaMin);
        fprintf(out, "buildMax: %lf queryMax: %lf updateMax: %lf ", baMax, qaMax, uaMax);
      }
    }
...
//            throwUsage("Histogram plot unsupported for >1 parameter. ");
  ...
  for (int i = 0; i < numIndices; i++) {
    if (trMap->begin()->second->size() > i) {
      fprintf(outfile, "%s ", trMap->begin()->second->at(i)->getIndexName());
      ...
      for (it = trMap->begin(); it != trMap->end(); it++) {
        ...
        fprintf(outfile, "build: %lf query: %lf update: %lf ", ba, qa, ua);
      }
      fprintf(outfile, "\n");
    }
  }
...
void printTickByTick(FILE* outfile, TimingResult* tr, int targetTrial) {
  ...
  for (int i = 0; i < tr->getNumTicks(); i++) {
    ...
    fprintf(outfile, "%d build: %lf query: %lf update: %lf numResults: %lf\n", i, ba, qa, ua, nr);
  }

}

enum PlotMode {
  AVG, AVG_BREAKDOWN, TICK_BY_TICK
};
...
//    std::vector<const char*>* wParams = NULL;
  ...
  if (outfileSet == false) {
    if (targetTrial == -1) {
      sprintf(tmpOutfile, "out/outfiles/%s%s.txt", experName, plotNames[(int) mode]);
    } else {
      sprintf(tmpOutfile, "out/outfiles/%sTrial%d%s.txt", experName, targetTrial, plotNames[(int) mode]);
    }
