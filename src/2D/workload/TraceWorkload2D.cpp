...
QueryRec* TraceWorkload2D::processQuery(int type,
    std::vector<Point2D*>* queryPoints, double xDelta, double yDelta, int k) {
  QueryType qType;
...
UpdateRec* TraceWorkload2D::processUpdate(int type, int id, double x, double y,
...
// }
  ...
  case 5:
    throw std::logic_error(
        "TraceWorkload2D: IMPLICIT_MOVE for \
                                internal use only. .");
    break;
  default:
...
// Read updates from trace.
  for (int i = 0; i < numUpdates; i++) {
    ...
    UpdateRec* rec = processUpdate(sUpdates[i].type, sUpdates[i].id,
        sUpdates[i].x, sUpdates[i].y, sUpdates[i].vx, sUpdates[i].vy,
        sUpdates[i].data);
...
//         std::cout << "second: " << *it->second << std::endl;
    ...
    // This concerning the below _Hashtable::count(k):
  ...
  updates->insert(updates->begin(), implicitUpdates.begin(),
      implicitUpdates.end());
...
TraceWorkload2D::TraceWorkload2D(const char* filename, bool binary/*,
 bool replayable*/) {
