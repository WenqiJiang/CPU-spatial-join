...
AbstractJoinRunner2D* JoinRunner2D::getRunner(JoinMethod* j, Workload2D* pw,
    bool copyPts, TimingResult* tr) {
...
void JoinRunner2D::runJoinOnWorkload(JoinMethod* j1, Workload2D* pw,
    TimingResult* tr) {
...
#if PAPI == 1
  PMU papi;
//  papi.add_event("PAPI_TOT_INS");
//  papi.add_event("PAPI_TOT_CYC");
//  papi.add_event("MEM_LOAD_RETIRED:HIT_LFB");

  papi.add_cache_miss_profile();

//	papi.add_cache_profile_I();
//	papi.add_cache_profile_II();
  papi.start();
#endif

  // drs: processing of ticks:
 ...
 * the tick. 
 */
bool JoinRunner2D::compareIndices(JoinMethod* j1, JoinMethod* j2,
    Workload2D* pw1, Workload2D* pw2) {

  LOG(DEBUG) << "JoinRunner2D: Comparing " << j1->getName() << " and "
      << j2->getName() << " on " << pw1->getName() << std::endl;
  ...
  while ( jr1->hasNextTick() ) {
    ...
    if ( !comp ) {
      std::cout << "ERROR: RESULTS DON'T MATCH at tick " << tickNum
          << std::endl;
