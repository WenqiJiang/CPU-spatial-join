...
CPP_VERSION='c++17'
ifndef GSL_INCL_DIR
CPPFLAGS_ALL_lib := -std=$(CPP_VERSION) -I$(HDR) -I$(CHK_HDR) \
	-I.. $(INCLUDES_FROM_PKGS) $(INCLUDES_FROM_DIRS) \
	-D__STDC_LIMIT_MACROS -D__STDC_CONSTANT_MACROS $(CPPFLAGS)
CPPFLAGS_ALL_cmd := -std=$(CPP_VERSION) -I$(HDR) -I$(CHK_HDR) \
	-I.. $(INCLUDES_FROM_PKGS) $(INCLUDES_FROM_DIRS) \
	-D__STDC_LIMIT_MACROS -D__STDC_CONSTANT_MACROS $(CPPFLAGS)
else
CPPFLAGS_ALL_lib := -std=$(CPP_VERSION) -I$(HDR) -I$(CHK_HDR) -I$(GSL_INCL_DIR) \
	-I.. $(INCLUDES_FROM_PKGS) $(INCLUDES_FROM_DIRS) \
	-D__STDC_LIMIT_MACROS -D__STDC_CONSTANT_MACROS $(CPPFLAGS)
CPPFLAGS_ALL_cmd := -std=$(CPP_VERSION) -I$(HDR) -I$(CHK_HDR) -I$(GSL_INCL_DIR) \
	-I.. $(INCLUDES_FROM_PKGS) $(INCLUDES_FROM_DIRS) \
	-D__STDC_LIMIT_MACROS -D__STDC_CONSTANT_MACROS $(CPPFLAGS)
endif
...
ifeq "$(PKG_KIND)" "lib"
ifeq "$(OSNAME)" "$(OSX_NAME)"
	$(CXX) -dynamiclib $(CXXFLAGS_ALL_lib) -o $@ -dylib $^ $(IMPORTS_ALL) 
	rm -f $(@D)/$(OPTLEVEL)-$(@F) ; rm -f $(@D)/$(NOPTLEVEL)-$(@F)
	ln -s $@ $(@D)/$(OPTLEVEL)-$(@F)
else ifeq "$(OSNAME)" "$(LINUX_NAME)"
	$(CXX) -shared -Wl,-soname,$(@F) $(CXXFLAGS_ALL_lib) -o $@ $^ -lc $(IMPORTS_ALL) 
	rm -f $(@D)/$(OPTLEVEL)-$(@F) ; rm -f $(@D)/$(NOPTLEVEL)-$(@F)
...
ifeq "$(OSNAME)" "$(OSX_NAME)"
	$(CXX) $(CXXFLAGS_ALL_cmd) -o $@ $^ $(IMPORTS_ALL) 
	rm -f $(@D)/$(OPTLEVEL)-$(@F) ; rm -f $(@D)/$(NOPTLEVEL)-$(@F)
	ln -s $@ $(@D)/$(OPTLEVEL)-$(@F)
else ifeq "$(OSNAME)" "$(LINUX_NAME)"
	$(CXX) $(CXXFLAGS_ALL_cmd) -o $@ $^ $(IMPORTS_ALL) 
