...
public:
  ...
  ExperimentRunID(int indexNum, int workloadNum, const char* indexName, const char* workloadName,
      double* indexParams, int numIndexParams, double* workloadParams, int numWorkloadParams) :
      indexNum(indexNum), workloadNum(workloadNum), indexName(indexName), workloadName(workloadName), indexParams(
          indexParams), numIndexParams(numIndexParams), workloadParams(workloadParams), numWorkloadParams(
          numWorkloadParams) {
 ...
 */
enum TimeComponent {
  BUILD = 0, QUERY = 1, UPDATE = 2, TOTAL = 3
};
...
private:
  ...
  struct TickTrial {
    long numResults;
    double times[4];
    double getTime(TimeComponent comp) {
      return times[(int) comp];
    }
...
//        void setIndexNum(int indexNum) {this->indexNum = indexNum; }
  ...
  TimingResult(ExperimentRunID* rID = NULL, int startTick = 0, const char* experimentName = "default",
      std::vector<const char*>* iParams = NULL, std::vector<const char*>* wParams = NULL, bool deserialized =
          false) :
      rID(rID), startTick(startTick), experimentName(experimentName), iParams(iParams), wParams(wParams), deserialized(
          deserialized) {
