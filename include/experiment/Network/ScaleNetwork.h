...
//    addIndex(getJoinMethod( new Stripes<Point2D, Key2D, SumAgg2D>(101, 101, 100000, 100000) ));
    ...
    // drs: my added
    addIndex(getJoinMethod( new SimpleGrid<Point2D, Key2D, SumAgg2D>(GRID_DFLT_CPS,
        DEFAULT_SPACE_SIZE, GRID_DFLT_BS, 50000) ));
    addIndex(getJoinMethod( new SGrid<Point2D, Key2D, SumAgg2D>(128,
        SGRID_DFLT_BS, DEFAULT_SPACE_SIZE) ));

    addWorkload(new TraceWorkload2D("traces/network/olnetwork.dat", true), 0);
    addWorkload(new TraceWorkload2D("traces/network/sgnetwork.dat", true), 0);
    addWorkload(new TraceWorkload2D("traces/network/sanetwork.dat", true), 0);
