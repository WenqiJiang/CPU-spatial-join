...
private:
  static const int BUFFSIZE = 10485760;  // 10 MB
  FILE* traceFile;
  ...
  QueryRec* processQuery(int type, std::vector<Point2D*>* queryPoints,
      double xDelta, double yDelta, int k);

  UpdateRec* processUpdate(int type, int id, double x, double y, double vx,
...
public:
  ...
  TraceWorkload2D(const char* const filename, bool binary/*,
