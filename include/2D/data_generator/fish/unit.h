...
public:
  ...
  float speed;  //
  bool informed;  // proportion of informed fishes (leaders)
  float gx, gy;  // goal position (preferred destination)

public:
  Unit() {
  }

  Unit(int _id, float _x, float _y, float _vx, float _vy, float _speed,
      bool _informed, float _gx, float _gy)
      : id(_id), x(_x), y(_y), vx(_vx), vy(_vy), speed(_speed), informed(
          _informed), gx(_gx), gy(_gy) {
  }
  ...
  inline void setX(const float& newX) {
    x = newX;
  }
  inline void setY(const float& newY) {
    y = newY;
  }
  inline void setVx(const float& newVx) {
    vx = newVx;
  }
  inline void setVy(const float& newVy) {
    vy = newVy;
  }
  inline void setGx(const float& newGx) {
    gx = newGx;
  }
  inline void setGy(const float& newGy) {
    gy = newGy;
  }
};
...
public:
  ...
  UnitEffects(int id)
      : oid(id), dX(0.0), dY(0.0) {
  }

  inline int getOid() const {
    return oid;
  }
  inline void setOid(const int& newOid) {
    oid = newOid;
  }
...
public:
  ...
  inline bool equals(const UnitAgg2D* const other) const {
    return true;
  }
  UnitAgg2D() {
  }
